 ...
 *   3. If no text in the band → "not detected" (don't crop empty space).
 */
...
export async function extractFooter(
  ...
  const rationale = `Found ${footerItems.length} text item(s) in the bottom 18%.${
    options?.allPages && options.allPages.length > 1
      ? " Compared against other pages for repetition signal."
      : ""
  }`;
...
function computeFooterTop(footerItems: TextItem[], pageHeight: number): number {
  const top = Math.min(...footerItems.map((t) => t.y));
  return Math.max(0, Math.floor(top - pageHeight * PADDING_RATIO));
}
...
/**
 * Score footer confidence. Pure function — testable. Bumps for:
 *   - copyright marks
 *   - page-number patterns
 *   - text that repeats across pages (very strong signal that it's a
 *     real footer and not body text that happens to be near the bottom)
 */
...
export function textRepeatsOnAnotherPage(
  ...
  for (const p of allPages) {
    ...
    const otherText = p.textItems
      .filter((t) => t.y >= p.height * FOOTER_BAND_RATIO)
      .map((t) => t.str)
      .join(" ");
    if (normalizeForRepetition(otherText).includes(target)) return true;
  }
  return false;
}
